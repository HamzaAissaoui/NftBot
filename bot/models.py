...
class Attributes(Base):
    ...
    # of the 10 cheapest sneakers in this interval
    cheapest_average_price = Column(Float(5))
    ...
    with Session as session:
        while max_ <= 30:
            attribute = Attributes(
                min_attribute_sum=min_, max_attribute_sum=max_, total_sneakers=0, cheapest_average_price=0)
...
def fill_scrapping_status():
    with Session as session:
        attribute = ScrappingStatus(
            last_scrapped=datetime.now(), finished_scrapping=False)
        session.add(attribute)
        session.commit()
